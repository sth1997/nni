...
if __name__ == "__main__":
    ...
    for trials in os.listdir(args.experiment_path + "/trials"):
        ...
        epoch = -1
        duration = -1.0
        with open(args.experiment_path + "/trials/" + trials + "/output.log", "r") as file_read:
        ...
        if epoch == -1:
            ...
            #exit()
            epoch = 0
        if epoch == 0 or duration < 0 or id == -1:
            error_trials += 1
            continue
    ...
    #Only successful trial will have output.log, so there won't be any error in output.log.
    error_trials = max_id - success_trials
    ...
    for i in range(total_trials):
        ...
        if i in id_duration_dict and id_duration_dict[i] >= 0:
